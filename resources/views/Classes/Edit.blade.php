...
<div class="mb-8 border-b border-[#E5E3DB] pb-5">
    ...
    <p class="mt-1 text-sm text-slate-500">Memperbarui catatan <span class="font-medium text-[#16213A]">{{ $class['name'] }}</span>.</p>
</div>

<form action="{{ route('SchoolClass.update', $class['id']) }}" method="POST" class="space-y-6 border border-[#E5E3DB] bg-white p-8">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <input type="text" id="major" name="major" value="{{ $class['major'] }}"
            class="w-full border border-[#D9D6CD] bg-[#FCFBF8] px-3.5 py-2.5 text-sm focus:border-[#A16207] focus:bg-white focus:outline-none">
    </div>

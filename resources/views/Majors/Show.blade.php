...
<div class="mt-3 border border-[#E5E3DB] bg-white">
    <div class="flex items-start justify-between border-b border-[#E5E3DB] bg-[#FCFBF8] px-8 py-6">
        <div>
            <p class="mb-1 text-[11px] uppercase tracking-[0.2em] text-[#A16207]">Lembar Data Jurusan</p>
    ...
    <dl class="divide-y divide-[#EFEDE6] text-sm">
        ...
        <div class="flex justify-between px-8 py-4">
            ...
            <dd class="font-medium text-[#16213A] max-w-xl text-right">{{ $major['description'] }}</dd>
        </div>
    </dl>

    <div class="flex justify-end gap-4 border-t border-[#E5E3DB] px-8 py-5">
        <a href="{{ route('Major.index') }}" class="px-4 py-2.5 text-sm font-medium text-slate-500 hover:text-[#16213A]">Kembali</a>
        <form action="{{ route('Major.destroy', $major['id']) }}" method="POST" onsubmit="return confirm('Hapus data jurusan ini?')">
            @csrf
            @method('DELETE')
            <button type="submit"
